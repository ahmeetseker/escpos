...
networkDevice.open(function (error) {
  if (error) {
    ...
    const menuItems = [
      ...
      { name: 'Ali baba köfte asds sdad dobulwpeynir gghghgghghhg hjjhhhgg', price: '250.00tl' ,quantity: 2 },
    ...
    escpos.Image.load(tux, function (image) {
      ...
      printer
        ...
        .size(0, 0)
        .style('B')
        .image(image, 'd24'); // 'd24' is the printing density, adjust as needed
      ...
      menuItems.forEach(menuItem => {
        ...
        if (menuItem.name.length > characterPerLineFontA) {
          name = menuItem.name.substring(0, characterPerLineFontA - menuItem.price.length - 5);
        }

        printer
          .align('CT')
          .pureText(`${menuItem.quantity}x ${String(name).padEnd(diff - menuItem.price.length - 3, ' ')}`)
      ...
      printer
        ...
        .newLine()
        .pureText(`KDV (18%): ${kdv.toFixed(2)}tl`)
        .newLine()
        .align('CT')
        .drawLine()
        .align('RT')
        .size(0.1,0.1)
        .pureText(`Genel Toplam: ${grandTotal.toFixed(2)}tl`)
        .newLine()
        .newLine()
        .cut()
        .close();
